...
llvm.func @triton_gen.split_barrier() {
  ...
  // CHECK:     llvm.call @_Z31intel_work_group_barrier_arriveii([[ZERO]], [[ONE]]) {passthrough = ["convergent"]} : (i32, i32) -> ()
  // CHECK-DAG: [[ZERO:%.*]] = llvm.mlir.constant(0 : i32) : i32
  // CHECK-DAG: [[ONE:%.*]] = llvm.mlir.constant(1 : i32) : i32
  // CHECK:     llvm.call @_Z29intel_work_group_barrier_waitii([[ZERO]], [[ONE]]) {passthrough = ["convergent"]} : (i32, i32) -> ()
...
llvm.func @triton_gen.dpas.f32(%c : vector<8xf32>, %a : vector<4xf32>, %b : vector<8xf32>) {
  ...
  // CHECK-NEXT: llvm.call @_Z43intel_sub_group_tf32_tf32_matrix_mad_k8_f32Dv4_fDv8_fS0_
