...
def ttgir_to_spirv(mod, options):
    if options.extern_libs:
        _add_external_libs(mod, options.extern_libs)
    spirv_code, share_memory_size = _triton.translate_triton_gpu_to_spirv(str(mod), asdict(options))  # noqa: E501
    mod.share_memory_size = share_memory_size
    return spirv_code


def spirv_to_spvbin(spirv: str, compute_capability: dict):
...
class XPUBackend(BaseBackend):
    ...
    def add_stages(self, stages, options):
        ...
        #                    lambda src: optimize_ttgir(ttir_to_ttgir(src, options.num_warps), options.num_stages, options))
        # stages["spirv"] = (lambda path: Path(path).read_text(),
        #                    lambda src: ttgir_to_spirv(src, options))
        # stages["spvbin"] = (lambda path: Path(path).read_bytes(),
        #                     lambda src: spirv_to_spvbin(src, options))
